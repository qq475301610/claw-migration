...
test('skill instructs preview-first push and pull workflow', async () => {
  ...
  assert.match(content, /ask for confirmation before any write operation/i);
  assert.match(content, /defaultRemote/i);
  assert.match(content, /remoteKey/i);
  assert.match(content, /source agent id from the package/i);
